...
def apply_custom_css():
    ...
    if is_dark:
        ...
        lavender = "#4a7fa8"
        cyan = "#38CED0"
        border = "rgba(102, 155, 188, 0.16)"
        border_hover = "rgba(102, 155, 188, 0.38)"
        shadow = "0 18px 42px rgba(3, 7, 18, 0.26)"
        sidebar = "rgba(11, 18, 31, 0.82)"
        nav_inactive = "rgba(255,255,255,0.03)"
        nav_active = "linear-gradient(135deg, #669bbc, #4a7fa8)"
        nav_hover = "linear-gradient(135deg, #4a7fa8, #669bbc)"
        glow_primary = "rgba(102, 155, 188, 0.08)"
        glow_secondary = "rgba(74, 127, 168, 0.06)"
    else:
        bg = "#F6F8FD"
        ...
        border_hover = "rgba(102, 155, 188, 0.24)"
        shadow = "0 20px 55px rgba(61, 76, 130, 0.10)"
        sidebar = "rgba(255,255,255,0.78)"
        nav_inactive = "rgba(102,155,188,0.03)"
        nav_active = "linear-gradient(135deg, #669bbc, #4a7fa8)"
        nav_hover = "linear-gradient(135deg, #4a7fa8, #669bbc)"
        glow_primary = "rgba(102, 155, 188, 0.08)"
        glow_secondary = "rgba(74, 127, 168, 0.06)"
    ...
    [data-testid="stAppViewContainer"], .stApp {{
        ...
        background: radial-gradient(circle at 18% 12%, {glow_primary}, transparent 26%), radial-gradient(circle at 82% 10%, {glow_secondary}, transparent 30%), radial-gradient(circle at 76% 78%, rgba(102,155,188,0.05), transparent 28%), {bg};
        color: {text};
    }}
    ...
    .stApp::before {{
        ...
        position: fixed;
        inset: 0;
        background-image: radial-gradient(rgba(102,155,188,0.035) 1.1px, transparent 1.1px);
    ...
    .hero-description .hero-icon {{ display: inline-flex; align-items: center; justify-content: center; width: 2.6rem; height: 2.6rem; border-radius: 12px; background: rgba(102,155,188,0.08); color: {primary}; }}
    .hero-description p {{ margin: 0; font-size: 1rem; color: {text_soft}; line-height: 1.5; }}
    .primary-cta, .stButton > button, .stFormSubmitButton > button {{ background: linear-gradient(135deg, {primary}, {lavender}) !important; color: white !important; border: none !important; border-radius: 14px !important; min-height: 3rem !important; padding: 0.75rem 1.7rem !important; font-weight: 700 !important; box-shadow: 0 12px 26px rgba(102,155,188,.22) !important; transition: transform .22s ease, box-shadow .22s ease, filter .22s ease !important; }}
    .primary-cta:hover, .stButton > button:hover, .stFormSubmitButton > button:hover {{ background: linear-gradient(135deg, {lavender}, {primary}) !important; color: white !important; transform: translateY(-3px) !important; box-shadow: 0 17px 35px rgba(102,155,188,.30) !important; filter: brightness(1.04) !important; }}
    ...
    .hero-cube {{ width: 150px; height: 150px; border-radius: 26px; background: rgba(255,255,255,0.28); border: 1px solid rgba(102,148,255,0.30); box-shadow: inset 0 0 26px rgba(255,255,255,0.72), 0 16px 32px rgba(102,155,188,0.12); display: flex; align-items: center; justify-content: center; text-align: center; backdrop-filter: blur(10px); }}
    ...
    .hero-sphere {{ position: absolute; border-radius: 50%; background: rgba(255,255,255,0.7); box-shadow: 0 0 0 8px rgba(102,155,188,0.06); }}
    ...
    .page-header-icon {{ display: inline-flex; align-items: center; justify-content: center; width: 52px; height: 52px; border-radius: 16px; background: linear-gradient(135deg, rgba(102,155,188,0.12), rgba(74,127,168,0.06)); color: {primary}; border: 1px solid rgba(102,155,188,0.12); }}
    ...
    .metric-card:hover {{ transform: translateY(-6px); border-color: {border_hover}; box-shadow: 0 20px 42px rgba(102,155,188,.12); }}
    .metric-card::after {{ content: ""; position: absolute; right: -14px; bottom: -24px; width: 110px; height: 70px; border: 1px solid rgba(102,155,188,0.18); border-top-left-radius: 80px; border-top-right-radius: 80px; border-bottom: none; transform: rotate(-18deg); opacity: 0.45; }}
    ...
    .metric-icon.blue {{ background: rgba(102,155,188,0.1); color: {primary}; }}
    .metric-icon.cyan {{ background: rgba(33,193,195,0.1); color: {cyan}; }}
    .metric-icon.purple {{ background: rgba(74,127,168,0.1); color: {lavender}; }}
    ...
    [data-testid="stSidebarNav"] a[aria-current="page"], [data-testid="stSidebarNav"] button[aria-current="page"] {{ background: {nav_active} !important; border: none !important; box-shadow: 0 12px 26px rgba(102,155,188,.18); color: white !important; }}
    ...
    .brand-logo {{ width: 40px; height: 40px; border-radius: 12px; display: inline-flex; align-items: center; justify-content: center; background: linear-gradient(135deg, {primary}, {lavender}); color: white; box-shadow: 0 12px 24px rgba(102,155,188,.18); }}
    ...
    [data-testid="stForm"] {{ background: rgba(255,255,255,0.88); border-radius: 24px; border: 1px solid {border}; box-shadow: 0 14px 38px rgba(102,155,188,.08); padding: 1.2rem 1rem 0.8rem; }}
    ...
    .stTabs [aria-selected="true"] {{ background: linear-gradient(135deg, rgba(102,155,188,0.10), rgba(74,127,168,0.08)) !important; color: {text} !important; border: 1px solid rgba(102,155,188,0.18); }}
